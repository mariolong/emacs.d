...
*** TODO elfeed-show/search: converting to org-mode

#+BEGIN_SRC emacs-lisp
  (defun elfeed-show-to-org ()
    (interactive)
    (url-to-org (get-elfeed-show-url)))

  (defun elfeed-search-to-org ()
    (interactive)
    (url-to-org (get-elfeed-search-url)))

  (defun url-to-org (url)
    (when url
      (eww (get-elfeed-show-url))
      (message "%s" "waiting ... for eww loading")
      (eww-to-org)))
#+END_SRC

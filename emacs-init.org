...
:init(load-theme 'gruvbox-dark-hard :no-confirm))
#+END_SRC
...
Source: https://www.emacswiki.org/emacs/ChangingCursorDynamically
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun hcz-set-cursor-color-according-to-mode ()
    ...
    (let ((color (cond (buffer-read-only "white")
                       (overwrite-mode "#5599aa")
                       (t "#f99157"))))
      (unless (and
...
字典：http://download.huzheng.org/zh_TW/

#+BEGIN_SRC emacs-lisp
  (bind-key "C-c w" 'kid-sdcv-to-buffer)
...
#+BEGIN_SRC emacs-lisp
  (use-package openwith
    ...
    (setq openwith-associations
          (list
           (list (openwith-make-extension-regexp '("flac" "mp3" "wav" "aif" "aiff" "m4a" "aac"))
                 "mpv" '(file))
...
#+BEGIN_SRC emacs-lisp
  (defun counsel-goto-recent-directory ()
    ...
    (let ((collection
           (delete-dups
            (append (mapcar #'(lambda (s) (substring (file-name-directory s) 0 -1))
                            recentf-list)
...
#+BEGIN_SRC emacs-lisp
  ...
  (setenv "QT_QPA_PLATFORMTHEME" "LXQt")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package hungry-delete
    ...
    :config
    (setq hungry-delete-chars-to-skip (concat hungry-delete-chars-to-skip "　")))
#+END_SRC
...
** replace-semi-toFull-form
#+BEGIN_SRC emacs-lisp :tangle no
  (defun replace-semi-to-full-form ()
    (interactive)
    (replace-string "“" "「")
    (replace-string "”" "」")
    )
...
#+BEGIN_SRC emacs-lisp
  (use-package aggressive-indent
    ...
    :config
    (dolist (mm '(
                  haskell-mode haskell-cabal-mode haskell-interactive-mode
                  python-mode
...
** nix-mode
From: [[https://github.com/NixOS/nix-mode][GitHub - NixOS/nix-mode: An Emacs major mode for editing Nix expressions.]]
Ref: [[https://matthewbauer.us/bauer/][bauer: an Emacs+Nix IDE]]
